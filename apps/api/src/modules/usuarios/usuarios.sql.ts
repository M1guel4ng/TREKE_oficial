// apps/api/src/modules/usuarios/usuarios.sql.ts
...
export const SQL = {
  ...
  asegurarTipoMovimiento: `
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM tipos_movimiento WHERE codigo='BONO_BIENVENIDA'
    );
  `,

  /*
   * ⚠ CAMBIO IMPORTANTE
   * La tabla movimientos ahora es:
   *   (id, monto, saldo_anterior, saldo_posterior, fecha_movimiento, billetera_user_id, tipo_mov_id)
   * ya NO tiene descripcion, tipo_referencia, referencia_id.
   */
  aplicarBonoBienvenida: `
    ...
    upd AS (
      ...
      RETURNING billetera.id AS billetera_id,
                b.saldo_disponible AS saldo_anterior,
                (b.saldo_disponible + 5) AS saldo_posterior
    )
  ...
  /*
   * ⚠ CAMBIO IMPORTANTE
   * La tabla movimientos YA NO tiene:
   *   descripcion, tipo_referencia, referencia_id
   * Ajustamos el SELECT a las columnas reales.
   */
  panelMovimientos: `
    SELECT
      m.id,
