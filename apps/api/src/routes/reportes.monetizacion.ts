...
r.get("/ingresos/total", async (_req, res, next) => {
  try {
    ...
    res.json({ ok: true, data: rows[0] || { compras_ok:0, bs_total:0, creditos_total:0 } });
  } catch (e) { next(e); }
});
...
r.get("/ingresos/por-mes", async (_req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT * FROM vw_monetizacion_ingresos_por_mes ORDER BY periodo ASC`);
...
r.get("/creditos/comprados-por-usuario", async (_req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT * FROM vw_creditos_comprados_por_usuario ORDER BY creditos_comprados DESC NULLS LAST`);
    res.json({ ok: true, data: rows });
  } catch (e) { next(e); }
});
...
r.get("/creditos/saldos", async (_req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT * FROM vw_saldo_creditos_usuario ORDER BY saldo_total DESC`);
    res.json({ ok: true, data: rows });
  } catch (e) { next(e); }
});
...
r.get("/suscripciones/adopcion", async (_req, res, next) => {
  try {
    ...
    res.json({ ok: true, data: rows[0] || { total_registros:0, activas:0, usuarios_con_suscripcion:0, ratio_activas:0 } });
  } catch (e) { next(e); }
});
